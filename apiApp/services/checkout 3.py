"""Checkout business logic moved out of views into the service layer.

Contains the order-fulfillment and cart-resolution helpers that were living
in ``apiApp/views.py`` (the Stripe webhook / redirect-fallback flow).
"""
import json
import logging
...
def _resolve_cart_code_from_session(session):
    ...
    if len(candidates) == 1:
        logger.info("Inferred cart_code from email+amount match: %s", candidates[0])
        return candidates[0]
