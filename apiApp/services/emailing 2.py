"""Order emailing: build template context and send order notifications.

Moved verbatim from the former single-file ``apiApp/services.py`` (domain
split). Depends only on ``apiApp.emails.send_email`` and Django settings, so
it sits on its own module to avoid import cycles (views -> services.emailing
-> emails).
"""
import logging
