...
def add_review(request):
    ...
    if Review.objects.filter(record_id=record_id, user__email=email).exists():
...
def get_record_reviews(request, record_id):
    ...
    reviews = Review.objects.filter(record=record).select_related('user', 'record')
...
def get_all_reviews(request):
    reviews = Review.objects.all().select_related('user', 'record')
    paginator = StandardResultsSetPagination()
    page = paginator.paginate_queryset(reviews, request)
    serializer = ReviewSerializer(page, many=True)
    return paginator.get_paginated_response(serializer.data)
