"""Checkout views: Stripe session creation, webhook, and completion flows."""
import json
import logging
...
def create_stripe_checkout_session(request):
    ...
    try:
        ...
        logger.info(
            ">>> checkout session created | id=%s cart_code=%s client_reference_id=%s",
...
def stripe_webhook(request):
    ...
    if event is None and getattr(settings, "ALLOW_UNVERIFIED_WEBHOOKS", False):
        try:
            event = json.loads(payload.decode() if isinstance(payload, (bytes, bytearray)) else payload)
            logger.warning("Proceeding with unverified Stripe webhook payload (ALLOW_UNVERIFIED_WEBHOOKS).")
        except Exception as e:
            last_error = e
    ...
    if event is None:
        if last_error:
            ...
            logger.warning("Stripe webhook rejected: event could not be parsed")
    ...
    if event_type in ('checkout.session.completed', 'checkout.session.async_payment_succeeded'):
        ...
        logger.warning("Stripe async payment failed event received.")
        pass
